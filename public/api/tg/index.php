...
function tg_ping_desk(array $item): void
{
    $lib = dirname(__DIR__) . '/desk/lib.php';
    if (!is_file($lib)) {
        return;
    }
    try {
        ...
        desk_enqueue_wake([
            ...
            'type' => $item['type'] ?? 'text',
            'text' => $text,
            'has_file' => !empty($item['local_path']) || !empty($item['file_id']),
        ], 'tg');
    } catch (Throwable $e) {
        // стол ещё не залит - бот не падает
    }
}
...
if ($method === 'GET' && ($rest === '' || $rest === 'health')) {
    ...
    tg_respond([
        ...
        'service' => 'tg-inbox',
        'mode' => 'pull+webhook',
        'code_rev' => '20260814-desk-wake',
        'items' => count($store['items']),
    ]);
}

// Webhook на REG.RU не ставим. Приём - VPS-поллер (getUpdates).
if ($method === 'GET' && ($rest === 'admin/setwebhook' || $rest === 'admin/setwebhook/')) {
    tg_require_admin($cfg);
    tg_respond(['ok' => false, 'error' => 'webhook disabled, vps poller owns getUpdates'], 410);
}
...
// Admin: overwrite only index.php (config.php не трогаем)
if (($method === 'POST' || $method === 'PUT') && ($rest === 'admin/deploy' || $rest === 'admin/deploy/')) {
    tg_require_admin($cfg);
    $raw = (string)file_get_contents('php://input');
    if (strpos($raw, '<?php') !== 0 || strpos($raw, 'function tg_ingest_update') === false) {
        tg_respond(['error' => 'payload looks invalid'], 400);
    }
    $dest = __DIR__ . '/index.php';
    $tmp = $dest . '.new';
    if (file_put_contents($tmp, $raw) === false) {
        tg_respond(['error' => 'write_failed'], 500);
    }
    if (!@rename($tmp, $dest)) {
        @unlink($tmp);
        tg_respond(['error' => 'rename_failed'], 500);
    }
    tg_respond(['ok' => true, 'wrote' => 'index.php', 'bytes' => strlen($raw)]);
}
