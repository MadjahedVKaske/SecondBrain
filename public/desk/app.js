...
function todaySectionIsOpen(key) {
  return TODAY_SECTION_OPEN[key] === true;
}
...
function gameWeeklyRankMarkerFor(objectType, objectId, fallback = "gray") {
  ...
  return `<span class="game-weekly-rank game-rank--${esc(rankId)}" title="${esc(label)}" aria-label="${esc(label)}"><b>${xp ? `<i aria-hidden="true">+</i><span>${xp}</span>` : "—"}</b></span>`;
}
...
function renderToday() {
  ...
  const longQuestMeta = task => {
    const match = String(task.notes || "").match(/^⌁\s*(\d{2})\.(\d{2})–(\d{2})\.(\d{2})\s*·\s*([^\n]+)/m);
    if (!match) return null;
    const [, startDay, startMonth, endDay, endMonth, cadence] = match;
    const year = Number(String(STATE.today).slice(0, 4));
    const iso = (y, month, day) => `${y}-${String(month).padStart(2, "0")}-${String(day).padStart(2, "0")}`;
    const start = iso(year, Number(startMonth), Number(startDay));
    const endYear = Number(endMonth) < Number(startMonth) ? year + 1 : year;
    const end = iso(endYear, Number(endMonth), Number(endDay));
    if (STATE.today < start || STATE.today > end) return null;
    const todayPrefix = `${String(STATE.today).slice(8, 10)}.${String(STATE.today).slice(5, 7)}`;
    const steps = (task.checklists || []).flatMap(list => list.items || []);
    const todaySteps = steps.filter(item => String(item.text || "").startsWith(todayPrefix));
    return {
      cadence: cadence.trim(),
      period: `${startDay}.${startMonth}–${endDay}.${endMonth}`,
      total: steps.length,
      done: steps.filter(item => item.done).length,
      todayTotal: todaySteps.length,
      todayDone: todaySteps.filter(item => item.done).length,
    };
  };
  ...
  const longQuestRows = longQuests.map(({ task, meta }) => {
    const realm = realmFor(gameSkillId("task", task.id));
    const progress = meta.total ? Math.round(meta.done * 100 / meta.total) : 0;
    return `<button type="button" class="game-long-quest game-long-quest--${realm.id}" data-id="${esc(task.id)}"><span><b>${esc(task.title)}</b><small>${esc(meta.period)} · ${esc(meta.cadence)} · сегодня ${meta.todayDone}/${meta.todayTotal}</small><span class="bar" aria-label="Прогресс ${progress}%"><i style="width:${progress}%"></i></span></span><em>${meta.done}/${meta.total}</em></button>`;
  }).join("");
  const longQuestContent = longQuestRows ? `<section class="game-long-quests" aria-label="Длительные квесты">${longQuestRows}</section>` : "";
  ...
  const allTodaySectionsOpen = todaySections.every(todaySectionIsOpen);
  const todaySectionControl = todaySections.length ? `<div class="game-today-controls"><button type="button" class="game-collapse-all" aria-controls="${todaySections.map(key => `game-today-${key}`).join(" ")}">${allTodaySectionsOpen ? "Свернуть всё" : "Развернуть всё"}</button></div>` : "";
  ...
  ${todaySectionControl}<div class="game-today-grid">
  ...
  const collapseDetails = [...el.querySelectorAll("[data-today-section]")];
  const syncTodaySectionControl = () => {
    const control = el.querySelector(".game-collapse-all");
    if (control) control.textContent = collapseDetails.every(details => details.open) ? "Свернуть всё" : "Развернуть всё";
  };
  collapseDetails.forEach(details => details.addEventListener("toggle", () => {
    saveTodaySectionOpen(details.dataset.todaySection, details.open);
    syncTodaySectionControl();
  }));
  el.querySelector(".game-collapse-all")?.addEventListener("click", () => {
    const open = !collapseDetails.every(details => details.open);
    collapseDetails.forEach(details => {
      details.open = open;
      saveTodaySectionOpen(details.dataset.todaySection, open);
    });
    syncTodaySectionControl();
  });
...
function heatHtml(checks, opts = {}) {
  ...
  for (let i = 0; i < days; i++) {
    ...
    const locked = !!opts.lockAll || (!!opts.lockToday && key === today);
    const lockedLabel = opts.lockLabel || "Выбери 10 или 30 минут выше";
    html += `<i class="${on} ${isT} ${locked ? "locked" : ""}" data-d="${key}" title="${locked ? esc(lockedLabel) : key}" ${locked ? 'aria-disabled="true"' : ""}></i>`;
...
function renderHabits() {
  ...
  const renderHabitDetail = h => {
    ...
    const cells = week.map((k, i) =>
      `<button type="button" class="wd ${checks[k] ? "on" : ""} ${k === STATE.today ? "today" : ""}" data-d="${k}" ${isMeditation ? 'disabled title="Для медитации выбери дату и длительность выше"' : isStepHabit && k === STATE.today ? 'disabled title="Отметь выполнение в карточке выше"' : ""}>${names[i]}</button>`
    ).join("");
    const meditationChoice = isMeditation
      ? (on
        ...
        : meditationVariant
          ? `<div class="habit-meditation-choice" role="group" aria-label="Выбрать длительность медитации"><span>Сегодня · выбери длительность</span><button type="button" class="${meditationVariant === "meditation-10" ? "is-selected" : ""}" data-variant="meditation-10">10 минут · +${Number(gameRankMeta("green").xp)} XP</button><button type="button" class="${meditationVariant === "meditation-30" ? "is-selected" : ""}" data-variant="meditation-30">30 минут · +${Number(gameRankMeta("blue").xp)} XP</button></div>`
          : `<div class="habit-meditation-choice" role="group" aria-label="Выбрать длительность медитации"><span>Сегодня</span><button type="button" data-variant="meditation-10">10 минут · +${Number(gameRankMeta("green").xp)} XP</button><button type="button" data-variant="meditation-30">30 минут · +${Number(gameRankMeta("blue").xp)} XP</button></div>`)
      : "";
    const yesterday = ymd(new Date(new Date(`${STATE.today}T12:00:00`).getTime() - 86400000));
    const meditationBackfill = isMeditation ? `<details class="habit-backfill">
      <summary>Отметить прошедший день</summary>
      <div class="habit-backfill-form">
        <label><span>Дата</span><input type="date" max="${esc(yesterday)}" value="${esc(yesterday)}" data-meditation-backfill-date/></label>
        <div class="habit-backfill-actions" role="group" aria-label="Длительность медитации за прошедший день"><span>Длительность</span><button type="button" data-meditation-backfill-variant="meditation-10">10 минут · +${Number(gameRankMeta("green").xp)} XP</button><button type="button" data-meditation-backfill-variant="meditation-30">30 минут · +${Number(gameRankMeta("blue").xp)} XP</button></div>
      </div>
      <small>Можно отметить любой прошедший день. Повторная запись за ту же дату не удвоит опыт.</small>
    </details>` : "";
    ...
    return `<section class="habit-detail" data-id="${esc(h.id)}">
      ...
      <details class="habit-history"><summary>История и отметки за неделю</summary>
        ...
        <div class="heat-wrap">
          ...
          <div class="heat">${heatHtml(checks, isMeditation ? { lockAll: true, lockLabel: "Для медитации выбери дату и длительность выше" } : { lockToday: isStepHabit })}</div>
        </div>
      </details>
    </section>`;
  };
  ...
  el.querySelectorAll(".habit-detail").forEach(box => {
    ...
    box.querySelectorAll("button[data-meditation-backfill-variant]").forEach(btn => btn.addEventListener("click", async () => {
      const date = box.querySelector("[data-meditation-backfill-date]")?.value || "";
      if (!date || date >= STATE.today) {
        toast("Выбери прошедший день");
        return;
      }
      const out = await api(`habits/${id}/check`, { date, on: true, variant_id: btn.dataset.meditationBackfillVariant });
      if (!out || !out.ok) { toast("Не удалось отметить медитацию"); return; }
      await load();
    }));
    ...
    box.querySelectorAll(".heat i").forEach(cell => {
      cell.addEventListener("click", async () => {
        ...
        if (isMeditation) {
          toast("Для медитации выбери дату и длительность в панели ритуала");
          return;
        }
        if (isStepHabit && d === STATE.today) {
          toast("Воду отмечай шагами в панели ритуала");
